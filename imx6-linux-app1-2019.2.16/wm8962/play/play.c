...
static void playback_test(char *name);
...
static void prg_exit(int code) 
{
	...
	if (handle)
		{
		printf("  snd_pcm_close_exec\n");
		snd_pcm_close(handle);
		}
	if (pidfile_written)
		{
		printf("  pidfile_written=%d\n",pidfile_written);
		remove (pidfile_name);
		}
	//只关闭设备，不退出进程
//	exit(code);
...
//	playback("time.wav");
//	printf("  app location 3\n");
//	playback_test(name);
//	playback(name);
//	printf("  app location 4\n");

//	printf("location 8\n");
	if (verbose==2)
		putchar('\n');
	snd_pcm_close(handle);//这里会调用hp_event,ctrl+c杀死进程也会调用
//	printf("location 9\n");
	handle = NULL;
	free(audiobuf);
      __end:
	snd_output_close(log);
//	snd_config_update_free_global_test();
...
static void playback_go_test(int fd, size_t loaded, off64_t count, int rtype, char *name)
{
	int l, r;
	off64_t written = 0;
	off64_t c;

	printf("\nhere_playback_go_test\n");
	printf("  app location 8\n");
	header(rtype, name);
	printf("  app location 9\n");
	set_params();
	
	printf("  chunk_bytes=%d\n",chunk_bytes);
	while (loaded > chunk_bytes && written < count && !in_aborting) {
		printf("  app location 10\n");
		if (pcm_write(audiobuf + written, chunk_size) <= 0)
			{
			printf("  app location 11\n");
			return;
			}
		written += chunk_bytes;
		loaded -= chunk_bytes;
...
//	init_stdin();
//	if (!name || !strcmp(name, "-")) {
//		printf("		playbacktest 1\n");
//		fd = fileno(stdin);
//		name = "stdin";
//	} else {
//		printf("		playbacktest 2\n");
//		init_stdin();
//		if ((fd = open(name, O_RDONLY, 0)) == -1) {
//			printf("		playbacktest 3\n");
//			perror(name);
//			prg_exit(EXIT_FAILURE);
//		}
//	}
...
//		printf("		playbacktest 4\n");
//		rhwparams.format = hwparams.format;
//		pbrec_count = calc_count();
//		playback_go_test(fd, 0, pbrec_count, FORMAT_AU, name);
//		goto __end;
//	}
	...
	ofs = test_vocfile(audiobuf);
//	if ((ofs = test_vocfile(audiobuf)) >= 0) {
//		printf("		playbacktest 5\n");
//		pbrec_count = calc_count();
//		voc_play(fd, ofs, name);
//		goto __end;
//	}
	printf("  app location 5\n");
	/* read bytes for WAVE-header */
	if ((dtawave = test_wavefile(fd, audiobuf, dta)) >= 0) {
		pbrec_count = calc_count();
		printf("  pbrec_count=%d\n",pbrec_count);
		printf("  dtawave=%d\n",dtawave);
		printf("  app location 7\n");
		playback_go_test(fd, dtawave, pbrec_count, FORMAT_WAVE, name);
		printf("  app location 6\n");
	} else {
		/* should be raw data */
		init_raw_data();
		pbrec_count = calc_count();
