...
#include <time.h>
//#include "../../alsa-lib-1.0.28/src/control/control_local.h"
	...
	if (level & LEVEL_BASIC) {
		...
		for (idx = 0; idx < count; idx++) {
			...
			case SND_CTL_ELEM_TYPE_BOOLEAN:
				...
				break;
			case SND_CTL_ELEM_TYPE_INTEGER:
				printf("%li", snd_ctl_elem_value_get_integer(control, idx));//yes
				break;
			case SND_CTL_ELEM_TYPE_INTEGER64:
...
char * numToStr(int value,char *p)
{
	char one,ten,hundred;
//	char *p;
	hundred=value/100;
	ten=(value%100)/10;
	one=value%10;
...
static int cset( int roflag, int keep_handle,int val)
	{
		int err;
		int i;
		...
		snd_ctl_elem_id_alloca(&id);
		snd_ctl_elem_value_alloca(&control);
		char volCtrStr[]="numid=24,iface=MIXER,name=Headphone Volume";
		char volCtr[]="0";
		int tmpVol=0;
		int direction=1;
		struct timespec request,remain;
		request.tv_nsec=300000000;
		request.tv_sec=0;
		printf("\nhere_cset\n");
	
		//val值转换成字符串存入vloCtr
		numToStr(val,volCtr);//num to str
	
		
	
		printf("volCtrStr=%s\n",volCtrStr);
		if (snd_ctl_ascii_elem_id_parse(id, volCtrStr)) {//获取数据到id里面 yes
...
//		show_control_id(id);//yes
	
		
		if (handle == NULL &&
			(err = snd_ctl_open(&handle, card, 0)) < 0) {//打开设备，handle为句柄 yes
			error("Control %s open error: %s\n", card, snd_strerror(err));
			return err;
		}
		printf("card=%s\n",card);
			
		snd_ctl_elem_info_set_id(info, id);//info->id=*id; yes
		if ((err = snd_ctl_elem_info(handle, info)) < 0) {//yes
			if (ignore_error)
				return 0;
		...
		snd_ctl_elem_info_get_id(info, id); 	/* FIXME: Remove it when hctl find works ok !!! *///yes
	//	printf("roflag=%d\n",roflag);
		if (!roflag) {
			printf("roflag=%d\n",roflag);
			snd_ctl_elem_value_set_id(control, id);//yes
			if ((err = snd_ctl_elem_read(handle, control)) < 0) {//读一些信息到control里面 yes
				if (ignore_error)
					return 0;
			...
			//音量应该就是这一步写到control里面的
		printf("volCtr=%s\n",volCtr);
	//	err = snd_ctl_ascii_value_parse(handle, control, info, argv[1]);//综合一些新的信息到control里面 yes
		err = snd_ctl_ascii_value_parse(handle, control, info, volCtr);
			if (err < 0) {
				if (!ignore_error)
					error("Control %s parse error: %s\n", card, snd_strerror(err));
				printf("keep_handle=%d\n",keep_handle);
				if (!keep_handle) {
					snd_ctl_close(handle);
					handle = NULL;
				}
	...
	//		printf("volume set 1\n");
	//		sleep(10);
			if ((err = snd_ctl_elem_write(handle, control)) < 0) {//把control写入，这句执行完后，有声音了 yes
				if (!ignore_error)
	...
	//		sleep(5);
			...
			printf("  cset will finish and close\n");
			snd_hctl_close(hctl);
		}
		return 0;
	}
